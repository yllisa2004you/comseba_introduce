...
const skillDescriptionContent = {
  ...
  JAVA: [
    ...
    "각 컴포넌트별 사용방법",
    "이벤트의 종류 및 적용 방법",
    "Window, Aciotn, Mouse, MouseMotion 이벤트",
    "Key,Focus,Item 이벤트",
  ],
  Python: [
    "파이썬의 개요, 특징 알아보기",
...
const makeSkillDescription = () => {
  ...
  let currentSkill = ""; //선택한 스킬 과목을 지정할 변수

  for (const box of skillBox) {
    const classList = box.className.split(" ");
    //box의 class 명들을 하나의 문자열로 가져온 후, 스페이스바를 기준으로 나누어 배열을 가져옴
    //과정이 헷갈린다면 consol.log(classList를 통해 확인해보자.)
    if (classList.includes("selected")) {
      //요소의 ㅡㄹ래스명에 selected가 있다면 선택된 요소이므로 요소의 text부분을 가져온다.
  ...
  skillDescriptionContent[currentSkill].forEach((Comment, idx) => {
    ...
    //css선택자를 활용하여 step안의 p태그 중 2번쨰를 선택할 수 있도록 한다.
    //백틱(`)으로 인덱스를 활용해서 몇스탭의 태그에 접근할 것인지 정할 수 있다.
    //이떄 배열은 0부터 시작하므로 idx에 1을 더해줘서 스탭의 단계를 맟춰준다.
...
//배열로 가져오기떄문에(4개) forEach를 통해서 하나씩 이벤트를 추가
skillBox.forEach((box) => {
  //addEventListner를 활용하면 선택한 요소에 여러가지 이벤트를 추가하는 것이 가능하다.

  let skill = ""; //목표로 할 skill을 저장
